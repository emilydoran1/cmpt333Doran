...
%% cut is placed before recursive call because after the functors are set you do not want to backtrack
...
%% cuts are placed before recursive calls because the program does not need to backtrack after the term was substituted
substitute(N,Old,New,Term,Term1) :-
    N > 0,
    arg(N,Term,Arg),
    !,
